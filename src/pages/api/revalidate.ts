...
import { getProjectMapClient, getLocaleClient } from '../../utilities/canvas/canvasClients';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const newPathsToRevalidate: string[] = [];
  const availableLocaleObjects = await getLocaleClient().get();
  const availableLocales: string[] = availableLocaleObjects.results.map(l => l.locale);
  pathsToRevalidate.forEach((path: string) => {
    if (path.includes(':locale')) {
      newPathsToRevalidate.push(...availableLocales.map(p => path.replace(':locale', p)));
    } else {
      newPathsToRevalidate.push(path);
    }
  });

  try {
    console.log(`Revalidating paths: '${newPathsToRevalidate}'`);
    await Promise.all(
      (newPathsToRevalidate || []).map(async pagePath => {
